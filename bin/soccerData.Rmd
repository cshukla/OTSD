...
```{r downloadData, cache=FALSE, message=FALSE}
#1992-1993 Season
jsonFile <-
...
## Plot the evolution of points required to win the EPL over time

```{r pointsEvolution, cache=FALSE, message=FALSE, error=FALSE, warning=FALSE}
ggplot(filter(epl, rank%in%c(1,2,3,4)), aes(x=Year, y=prcntPts, color=rank, group=rank)) + 
  geom_smooth(method = 'loess', se=TRUE, alpha=0.1) + 
  theme_bw() + theme(axis.text.x = element_text(angle = 315, hjust = 0))
